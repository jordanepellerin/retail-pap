...
function LigneRecap({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3 py-1.5">
      <span className="shrink-0 font-sans text-[11px] uppercase tracking-[0.14em] text-ardoise">
        {label}
      </span>
      <span className="text-right font-sans text-[12px] text-encre">{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex min-h-full flex-col p-5">
      <div className="flex justify-center pt-2">
        <div className="anim-pop-in flex h-16 w-16 items-center justify-center rounded-full bg-bordeaux">
      ...
      <div className="mt-5 border border-filet bg-blanc p-4">
        <p className="mb-2 font-sans text-[10px] uppercase tracking-[0.2em] text-bordeaux">
          Votre tenue
        </p>
        <div className="divide-y divide-filet">
          {tenue.map((a) => (
            <div key={a.id} className="flex items-baseline justify-between gap-3 py-1.5">
              <span className="min-w-0 truncate font-serif text-[14px] text-encre">{a.nom}</span>
              <span className="shrink-0 font-sans text-[11px] tracking-[0.06em] text-ardoise">
                {formatPrix(a.prix)}
              </span>
            </div>
          ))}
