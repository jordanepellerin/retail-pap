...
function CarteProduit({ article }: { article: Article }) {
  ...
  return (
    <article className="group">
      ...
      <div className="pt-3">
        <h3 className="font-sans text-[13px] text-noir-encre">{article.nom}</h3>
        <p className="mt-0.5 font-sans text-[12px] font-light text-gris-texte">{article.couleur}</p>
        <p className="mt-1 font-sans text-[13px]">
          {remise ? (
            <>
              ...
              <span className="ml-2 text-gris-texte line-through">{formatPrix(article.prix)}</span>
            </>
          ) : (
